if imnodeed == nil or imgui.set_next_window_size == nil or sdk.hook_vtable == nil then
...
local function get_localPlayerGO()
    ...
    elseif gn == "dmc5" then
        ...
        return player_comp:call("get_GameObject"), player_comp
...
local function get_localPlayer()
    if gn == "dmc5" then
        ...
        return player_manager:call("get_manualPlayer")
    end
end

local function get_firstEnemy()
    if gn == "dmc5" then
        local enemy_manager = sdk.get_managed_singleton(sdk.game_namespace("EnemyManager"))
        local enemyNum = enemy_manager:call("get_enemyNum")
        if enemyNum > 0 then
            local enemy_comp = enemy_manager:call("getEnemy(System.Int32)", 0)
            return enemy_comp:call("get_GameObject"), enemy_comp
        end
    end
end
    ...
    if imgui.tree_node(name) then
        ...
        if uvar ~= nil then
            if imgui.tree_node("UVar:" .. uvar:get_Name()) then
                ...
                local uvar_addr = uvar:get_address()
                local uvar_Bool = uvar:get_Bool()
                if not g_temp_map then
                    g_temp_map = {}
                end
                if not g_temp_map[uvar_addr] then
                    g_temp_map[uvar_addr] = {}
                end
                if uvar_Bool then
                    table.insert(g_temp_map[uvar_addr], uvar_Bool and "True" or "False")
                end
                if #g_temp_map[uvar_addr] > 20 then
                    g_temp_map[uvar_addr] = {}
                end
                
                imgui.text(string.format("Bool: %s", table.concat(g_temp_map[uvar_addr], ",")))
...
local function display_bhvt_array(tree, node, bhvt_array, tree_func, predicate, on_duplicate)
    ...
    for i=0, bhvt_array:size()-1 do
        ...
        imgui.button(string.format("[%03d]", i))
        imgui.same_line()
...
local via_userdata_TypeKind = {
    ["Boolean"] = {
        ["default"] = 2,
        ["flags"] = "FamANDAssem | Family | Static | Literal | ExposeMember | HasDefault",
        ["id"] = 3122,
        ["getter"] = "get_Bool",
        ["init_data_index"] = 3,
        ["offset_from_base"] = "0x10",
        ["offset_from_fieldptr"] = "0x0",
        ["type"] = "via.userdata.TypeKind"
    },
    ["C16"] = {
        ["default"] = 14,
        ["flags"] = "FamANDAssem | Family | Static | Literal | ExposeMember | HasDefault",
...
local via_userdata_TypeKind_Map = {}
for k, v in pairs(via_userdata_TypeKind) do
    via_userdata_TypeKind_Map[v.default] = {
        name = k,
        value = v,
    }
end

local function get_value_Variable(uvar)
    local kindType = uvar:get_TypeKind()
    local kind = via_userdata_TypeKind_Map[kindType]
    local v = nil
    pcall(function()
        v = uvar:call(kind.value.getter)
    end)
    if v ~= nil then
        return v
    end
...
local function display_UserVariables(uvarList)
    local isDump = false
    if imgui.button("Dump UserVariables") then
        local strList = {}
        for i, uvar in ipairs(uvarList) do
            local id = tostring(uvar)
            local kindType = uvar:get_TypeKind()
            local kind = via_userdata_TypeKind_Map[kindType]
            local name = uvar:get_Name()
            local value = get_value_Variable(uvar)
            table.insert(strList, string.format("%s, (%s), %s(%s)", tostring(name), tostring(value), tostring(kind.name), tostring(kindType)))
        end
        imgui.set_clipboard(table.concat(strList, "\n"))
    end
    imgui.begin_table("##UserVariablesTable"..tostring(uvarList), 2, 1920)
    imgui.table_setup_scroll_freeze(0, 1);
    imgui.table_setup_column("Name", 16, 200);
    imgui.table_setup_column("Value", 16, 50);
    ...
    for i, uvar in ipairs(uvarList) do
        ...
        -- imgui.table_next_column()
        -- imgui.text(string.format("%s (%s)", tostring(kind.name), tostring(kindType)))
        -- if isDump then
        --     log.debug(string.format("%s, (%s)", tostring(name), tostring(value)))
        -- end
        imgui.pop_id()
    end
    imgui.end_table()
...
draw_node_children = function(i, node, seen, active)
    ...
    for j, child_id in ipairs(node_descriptor.children) do
        ...
        out_dim_requirements.y = out_dim_requirements.y + node_dim_requirements.y
    end
...
draw_node = function(i, seen)
    ...
    local node_descriptor = custom_tree[i]
    local node = draw_standard_node("[" .. tostring(i) .. "]" .. node_descriptor.name, custom_id, function()
...
local function move_to_node(id)
    imnodeed.CenterNodeOnScreen(id)
end
...
local function draw_world_line(pos, pos1)
    local plane = sdk.create_instance("via.Plane", true):add_ref()
    plane:call(".ctor")	
    plane:set_field("dist", 10000.0)
    local drawLine = sdk.find_type_definition("app.Debug"):get_method("DrawLine(via.vec3, via.vec3, System.Single, via.Plane, via.Color, System.String, System.Single)")
    local dir = pos1 - pos
    local color = sdk.find_type_definition("via.Color"):get_field("Red")
    drawLine:call(nil, pos, (pos1 - pos):normalized(), dir:length(), plane, color)
end

local function draw_world_point(pos)
    local pos_s = draw.world_to_screen(pos)
    if pos_s then
        draw.line(pos_s.x - 10, pos_s.y     , pos_s.x + 10, pos_s.y,      0xFF0000FF)
        draw.line(pos_s.x,      pos_s.y - 10, pos_s.x,      pos_s.y + 10, 0xFF0000FF)
    end
end
...
local function draw_stupid_editor(name)
    ...
    --     local cameraMat = camera:call("get_WorldMatrix")
    --     local cameraPos = cameraMat[3]:to_vec3()
    --     imgui.text("cameraPos: [" .. cameraPos.x .. ", " .. cameraPos.y .. ", " .. cameraPos.z .. "]")
    --     -- local cameraPos_s = draw.world_to_screen(cameraPos)
    --     -- draw.line(cameraPos_s.x, cameraPos_s.y, width/2, height/2, 0xFF0000FF)
    --     -- imgui.text("\n[" .. cameraPos_s.x .. ", " .. cameraPos_s.y .. ", " .. cameraPos_s.z .. "]")
        
    --     local player_lockOnPos = player:call("getLockOnPos")
    --     local player_basePos = player:call("getBasePos")
    --     -- draw_world_line(cameraPos, player_lockOnPos)
        
    --     -- draw_world_pos(player_lockOnPos)
    --     -- draw_world_pos(player_basePos + player_comp:call("getBasePos"))
    --     draw_world_pos(player:call("get_LockOnOffset"))

    --     local enemy, enemy_comp = get_firstEnemy()
    --     if enemy and enemy_comp then
    --         local enemy_lockOnPos = enemy_comp:call("getLockOnPos")
    --         draw_world_pos(enemy_lockOnPos)
    --         -- draw_world_line(enemy_lockOnPos, player_lockOnPos)
            
    --         local enemy_lockOnPos_s = draw.world_to_screen(enemy_lockOnPos)
    --         local player_lockOnPos_s = draw.world_to_screen(player_lockOnPos)
    --         if player_lockOnPos_s and enemy_lockOnPos_s then
    --             draw.line(player_lockOnPos_s.x, player_lockOnPos_s.y, enemy_lockOnPos_s.x, enemy_lockOnPos_s.y, 0xFF0000FF)
    --         end

    --         local cx = (enemy_lockOnPos.x + player_lockOnPos.x)/2
    --         local cy = (enemy_lockOnPos.y + player_lockOnPos.y)/2
    --         local cz = (enemy_lockOnPos.z + player_lockOnPos.z)/2
    --         local center = Vector3f.new(cx, cy, cz)
            
    --         local p = enemy_lockOnPos + player_basePos
    --         local center = Vector3f.new(p.x/2, p.y/2, p.z/2)
    --         draw_world_point(center)
    --     end
    -- end
    ...
    local motion_fsm2 = playerGO and playerGO:call("getComponent(System.Type)", sdk.typeof("via.motion.MotionFsm2")) or nil
    ...
    if imgui.begin_menu_bar() then
        ...
        -- Selected node
        imgui.separator()
        imgui.text(tostring(imnodeed.GetSelectedObjectCount()) .. " selected nodes")
    ...
    if cfg.show_side_panels then
        local changed, lw, rw = imgui.Splitter(true, 4.0, cfg.sidePanelWidth, cfg.editorPanelWidth, 50.0, 50.0);
        if changed then
    ...
    imnodeed.Begin("My Editor", Vector2f.new(0, 0));
